...
`MViewer`는 로컬 폴더 안의 이미지와 영상을 빠르게 탐색하고, 미리보기하고, 필요한 몇 가지 보조 작업까지 처리할 수 있는 Tauri 기반 미디어 뷰어 데스크톱 앱입니다. 웹 브라우저가 아닌 데스크톱 런타임에서 동작하므로 로컬 파일 경로 접근이 필요한 미디어 감상과 정리 흐름에 적합합니다.
